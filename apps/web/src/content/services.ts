 ...
 * pricing — see docs/PROJECT_REQUIREMENTS.md §7. This file's shape mirrors
 * docs/DATABASE.md's `serviceCategories`/`serviceItems` collections so
 * Phase 5 can replace it with a live API call with minimal rework; nothing
 * that reads from `SERVICE_CATEGORIES` below should need to change shape.
 */
...
export function getStartingPrice(category: ServiceCategory): number {
  return Math.min(...category.items.map((item) => item.priceRupees));
}
